...
from training.fl_trainer import FedALConfig, FedALTrainer


def parse_args():
    parser = ArgumentParser(
        description="Federated Active Learning training for medical image segmentation"
    )
    ...
    # >>> AL parameters
    parser.add_argument("--al-strategy", default="entropy",
                        choices=["entropy", "random"],
                        help="Active learning query strategy")
    parser.add_argument("--al-budget-per-round", default=5, type=int,
                        help="Number of samples to label per client per FL round")
    parser.add_argument("--al-initial-labeled", default=10, type=int,
                        help="Number of initially labeled samples per client")
    # <<< AL parameters
...
def train_entry():
    ...
    config = FedALConfig(
        ...
        al_budget_per_round=args.al_budget_per_round,
        al_initial_labeled=args.al_initial_labeled,
        # Optimizer
